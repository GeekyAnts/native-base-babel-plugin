...
var fs = require("fs"); // Utility functions


function updateProviderIdsInJSONFile(data, filename) {}

function createJSXAttributeNode(name, value) {
  var providerIds = [];

  if (fs.existsSync(process.cwd() + "/node_modules/native-base/lib/module/utils/map/providerIds.json")) {
    var _JSON$parse;

    var data = fs.readFileSync(process.cwd() + "/node_modules/native-base/lib/module/utils/map/providerIds.json").toString("utf8");
    providerIds = (_JSON$parse = JSON.parse(data)) === null || _JSON$parse === void 0 ? void 0 : _JSON$parse.providers;
  } // console.log("called proviederID", name, value);


  providerIds.push(value);
  fs.writeFileSync(process.cwd() + "/node_modules/native-base/lib/module/utils/map/providerIds.json", JSON.stringify({
...
var updateFile = function updateFile(platform, data) {
  ...
  if (fs.existsSync(process.cwd() + "/node_modules/native-base/lib/module/utils/map/providerIds.json")) {
    ...
    var _data = fs.readFileSync(process.cwd() + "/node_modules/native-base/lib/module/utils/map/providerIds.json").toString("utf8");

    providerIds = (_JSON$parse2 = JSON.parse(_data)) === null || _JSON$parse2 === void 0 ? void 0 : _JSON$parse2.providers;
  } // console.log(providerIds, "providerIds");
  // console.log(componentsList, "componentsList");
  // console.log(platform, data, "componentsMap");
  // console.log("Which  proviederID ?", updatedResolvedStyledMap, providerIds);
  ...
  var modifiedData = "export default ".concat(JSON.stringify(updatedResolvedStyledMap, null, 2)); // console.log("modifiedData", modifiedData);
    ...
    visitor: {
      Program: function Program(path) {
        ...
        if (!filePath.includes("/node_modules/")) {
              ...
              if (Object.keys(componentsList).includes(jsxOpeningElementPath.node.name.name)) {
                if (jsxOpeningElementPath.node.name.name === "NativeBaseProvider") {
                  if (jsxOpeningElementPath.node.attributes) {
                    jsxOpeningElementPath.node.attributes.push(createJSXAttributeNode("providerId", uniqueId("nbBootTime-")));
                  } else {
                    jsxOpeningElementPath.node.attributes = [createJSXAttributeNode("providerId", uniqueId("nbBootTime-"))];
                  }
                } else {
          ...
          }); // console.log("called update", componentsMap);
